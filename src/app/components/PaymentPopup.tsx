    ...
    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 sm:mx-auto overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[95vh] overflow-y-auto">
      ...
      <div className="bg-gradient-to-r from-red-600 to-red-700 px-4 py-4 sm:px-6 sm:py-6">
        <div className="flex items-center justify-center space-x-3">
          <div className="bg-white rounded-lg px-4 py-2 sm:px-5 sm:py-2.5 shadow-md">
            <span className="text-red-600 font-bold text-xl sm:text-2xl tracking-widest">KHQR</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <p className="text-gray-600 text-xs sm:text-sm mb-2 uppercase tracking-wide">ចំនួនទឹកប្រាក់</p>
          ...
          <div className="aspect-square bg-white rounded-xl p-4 sm:p-6 mb-4 sm:mb-6 shadow-lg relative">
            ...
            {isLoading ? (
              <div className="w-full h-full flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 font-medium text-sm sm:text-base">កំពុងបង្កើត KHQR...</p>
              </div>
            ) : error ? (
              <div className="w-full h-full flex flex-col items-center justify-center text-center p-4 sm:p-6">
                <p className="text-red-600 font-medium mb-3 sm:mb-4 text-sm sm:text-base">⚠️ {error}</p>
                <Button onClick={initializePayment} variant="outline" className="text-xs sm:text-sm">
                  ព្យាយាមម្តងទៀត
                </Button>
              </div>
            ) : qrCodeUrl ? (
              <>
                ...
                <div className="absolute bg-gradient-to-br from-green-400 to-green-600 rounded-full p-2.5 sm:p-3.5 shadow-xl border-2 sm:border-4 border-white left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
                  <span className="text-white font-bold text-2xl sm:text-3xl drop-shadow-md">៛</span>
                </div>
              </>
            ) : null}
          </div>
          ...
          <div className="text-center space-y-2">
            ...
            {paymentStatus === 'PAID' && (
              <div className="flex items-center justify-center gap-2 text-green-600">
                ...
                <p className="font-bold text-base sm:text-lg">បានទូទាត់ជោគជ័យ!</p>
              </div>
            )}
            {paymentStatus === 'FAILED' && (
              <p className="text-red-600 font-semibold text-sm sm:text-base">ការទូទាត់បានបរាជ័យ</p>
            )}
          </div>
        </div>
